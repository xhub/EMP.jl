...
function test_variable_no_bound(ModelType, VariableRefType)
    m = EMP.Model(); mp = MathPrgm(m)
    @variableMP(mp, nobounds)
...
function test_variable_lower_bound_rhs(ModelType)
    ...
    @test isequal(m.model_ds[:lbonly], lbonly)
    JuMP.delete_lower_bound(lbonly)
    @test !JuMP.has_lower_bound(lbonly)
    # Name already used
    @test_throws ErrorException @variableMP(mp, lbonly)
end

function test_variable_lower_bound_lhs(ModelType)
    m = EMP.Model(); mp = MathPrgm(m)
    @variableMP(mp, 0 <= lblhs, Bin)
    ...
    @test isequal(m.model_ds[:lblhs], lblhs)
end

function test_variable_upper_bound_rhs(ModelType)
    m = EMP.Model(); mp = MathPrgm(m)
    @variableMP(mp, ubonly <= 1, Int)
    ...
    @test isequal(m.model_ds[:ubonly], ubonly)
...
function test_variable_upper_bound_lhs(ModelType)
    ...
    @test isequal(m.model_ds[:ublhs],ublhs)
end
...
function test_variable_interval(ModelType)
    ...
    m = EMP.Model(); mp = MathPrgm(m)
    @variableMP(mp, 0 <= bothb1 <= 1)
    has_bounds(bothb1, 0.0, 1.0)
    @variableMP(mp, 0 ≤  bothb2 ≤  1)
    has_bounds(bothb2, 0.0, 1.0)
    @variableMP(mp, 1 >= bothb3 >= 0)
    has_bounds(bothb3, 0.0, 1.0)
    @variableMP(mp, 1 ≥  bothb4 ≥  0)
    has_bounds(bothb4, 0.0, 1.0)
    @test_macro_throws ErrorException @variableMP(mp, 1 ≥ bothb5 ≤ 0)
    @test_macro_throws ErrorException @variableMP(mp, 1 ≤ bothb6 ≥ 0)
end

function test_variable_fix(ModelType)
    m = EMP.Model(); mp = MathPrgm(m)
    @variableMP(mp, fixed == 1.0)
...
function test_variable_custom_index_sets(ModelType)
    m = EMP.Model(); mp = MathPrgm(m)
    @variableMP(mp, onerangeub[-7:1] <= 10, Int)
    @variableMP(mp, manyrangelb[0:1, 10:20, 1:1] >= 2)
    ...
    s = ["Green","Blue"]
    @variableMP(mp, x[i=-10:10, s] <= 5.5, Int, start=i+1)
    @test 5.5 == @inferred JuMP.upper_bound(x[-4, "Green"])
    test_variable_name(x[-10, "Green"], "x[-10,Green]")
    @test JuMP.start_value(x[-3, "Blue"]) == -2
    @test isequal(m.model_ds[:onerangeub][-7], onerangeub[-7])
    @test_throws KeyError m.model_ds[:foo]
end

function test_variable_anonymous(ModelType)
    m = EMP.Model(); mp = MathPrgm(m)
    @test_throws ErrorException @variableMP(mp, [(0, 0)])  # #922
    x = @variableMP(mp, [(0, 2)])
...
function test_variable_is_valid_delete(ModelType)
    m = EMP.Model(); mp = MathPrgm(m)
    @variableMP(mp, x)
    @test JuMP.is_valid(m.model_ds, x)
    JuMP.delete(m.model_ds, x)
    @test !JuMP.is_valid(m.model_ds, x)
    second_m = EMP.Model(); mp = MathPrgm(m)
    @test_throws Exception JuMP.delete(second_m.model_ds, x)
end
...
function test_variable_repeated_elements(ModelType)
    ...
    @test_throws ErrorException (
        @variableMP(mp, unused_variable[index_set], container=DenseAxisArray))
    @test_throws ErrorException (
        @variableMP(mp, unused_variable[index_set], container=SparseAxisArray))
    @test_throws ErrorException (
        @variableMP(mp, unused_variable[index_set, [1]], container=DenseAxisArray))
    @test_throws ErrorException (
        @variableMP(mp, unused_variable[index_set, [1]], container=SparseAxisArray))
end
...
function test_variable_oneto_index_set(ModelType, VariableRefType)
    ...
    m = EMP.Model(); mp = MathPrgm(m)
    auto_var = @variableMP(mp, [Base.OneTo(3), 1:2], container=Auto)
    @test auto_var isa Matrix{VariableRefType}
    @test (3, 2) == @inferred size(auto_var)
    array_var = @variableMP(mp, [Base.OneTo(3), 1:2], container=Array)
    @test array_var isa Matrix{VariableRefType}
    @test (3, 2) == @inferred size(array_var)
    denseaxisarray_var = @variableMP(mp, [Base.OneTo(3), 1:2], container=DenseAxisArray)
...
function test_variable_name(ModelType)
    ...
    JuMP.set_name(x, "y")
    @test JuMP.variable_by_name(m.model_ds, "x") isa Nothing
    test_variable_name(x, "y")
    y = @variableMP(mp, base_name="y")
    err(name) = ErrorException("Multiple variables have the name $name.")
    @test_throws err("y") JuMP.variable_by_name(m.model_ds, "y")
    JuMP.set_name(y, "x")
    test_variable_name(x, "y")
    test_variable_name(y, "x")
    JuMP.set_name(x, "x")
    @test_throws err("x") JuMP.variable_by_name(m.model_ds, "x")
    @test JuMP.variable_by_name(m.model_ds, "y") isa Nothing
    JuMP.set_name(y, "y")
    test_variable_name(x, "x")
...
function test_variable_start_value_on_empty(ModelType)
    m = EMP.Model(); mp = MathPrgm(m)
    @variableMP(mp, x[1:4,  1:0,1:3], start = 0)  # Array{VariableRef}
    @variableMP(mp, y[1:4,  2:1,1:3], start = 0)  # DenseAxisArray
    @variableMP(mp, z[1:4,Set(),1:3], start = 0)  # SparseAxisArray
...
function test_variable_denseaxisarray_slices(ModelType, VariableRefType)
    ...
    m = EMP.Model(); mp = MathPrgm(m)
    @variableMP(mp, x[1:3, 1:4, 1:2], container=DenseAxisArray)
    @variableMP(mp, y[1:3, -1:2, 3:4])
    @variableMP(mp, z[1:3, -1:2:4, 3:4])
    @variableMP(mp, w[1:3, -1:2,[:red, "blue"]])
...
function test_variable_unsigned_index(ModelType)
    ...
    t = UInt(4)
    @variableMP(mp, x[1:t])
    @test 4 == @inferred num_variables(m.model_ds)
end

function test_variable_symmetric(ModelType)
    m = EMP.Model(); mp = MathPrgm(m)

    @variableMP mp x[1:2, 1:2] Symmetric
    @test x isa Symmetric
    @test x[1, 2] === x[2, 1]
    @test m.model_ds[:x] === x

    y = @variableMP mp [1:2, 1:2] Symmetric
...
@testset "Variables for JuMP.Model" begin
    ...
    @testset "all_variables" begin
        ...
        @variableMP(mp, x)
        @variableMP(mp, y)
        @test [x, y] == @inferred JuMP.all_variables(m.model_ds)
    end
